...
object ConfigUtil {
    ...
    fun getByPlugin(plugin: String):List<JsonConfig> {
        return cache[plugin.lowercase()]?.values?.toList() ?: listOf()
    }

    fun getByPluginAndName(plugin: String, name: String): JsonConfig? {

        if (!cache.containsKey(plugin.lowercase())) {
            return null
        }

        return cache[plugin.lowercase()]!![name.lowercase()]
    }
    ...
    fun <T> register(name: String,plugin: String,dataFolder: File,config: JsonConfig):T {
        ...
        cache.putIfAbsent(plugin.lowercase(),hashMapOf())
        cache[plugin.lowercase()]!![name] = toReturn
